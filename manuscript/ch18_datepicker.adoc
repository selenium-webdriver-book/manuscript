...
`ReactCalendarControls` implements `Consumer<Browser>` so its constants can be used as the parameter for the constructor of `CalendarPicker` class in Listing <<CalendarPicker>>. And `CalendarPicker` class is used as the parameters when constructing an instance of `Datepicker` class, as shown in Listing for `BoostrapDatepickerPage`.
